...
int send_file(int socket_fd, const char* filename) {
    ...
    send(socket_fd, &file_info, sizeof(FileInfo), 0);
...
void get_server_response(int sock, char* buffer, int buffer_size) {
    ...
    if (valread > 0) {
        ...
        strcpy(buffer, "Сервер отключился");
    } else {
        perror("recv");
    ...
    while (1) {
        ...
        switch (choice) {
            case 1: {
                ...
                sprintf(buffer, "CALC %s %.2lf %.2lf", operation, num1, num2);
                
                send(sock, buffer, strlen(buffer), 0);
                printf("Отправлено: %s\n", buffer);
                
                get_server_response(sock, buffer, BUFFER_SIZE);
