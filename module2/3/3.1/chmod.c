...
void printMode(mode_t st_mode){
    ...
    printf("\n");
    mode_t new_mode = S_IRUSR;
    for(int i = 0; i < 9; i++){
        if(new_mode & st_mode){printf("1");}else{printf("0");}
        new_mode = new_mode >> 1;
        if(i == 2 || i == 5){printf("-");}
    }
    printf("\n");
    int count = 0;
    if(S_IRUSR & st_mode){count+=4;}
    if(S_IWUSR & st_mode){count+=2;}
    if(S_IXUSR & st_mode){count+=1;}
    printf("%d", count);
    count = 0;
    if(S_IRGRP & st_mode){count+=4;}
    if(S_IWGRP & st_mode){count+=2;}
    if(S_IXGRP & st_mode){count+=1;}
    printf("%d", count);
    count = 0;
    if(S_IROTH & st_mode){count+=4;}
    if(S_IWOTH & st_mode){count+=2;}
    if(S_IXOTH & st_mode){count+=1;}
    printf("%d", count);
    printf("\n");

    
}
mode_t FileMod(char* filename){
    struct _stat buf;
    int result;
...
void parsingPrint(char* str, int len_str, char* name_file){
    int count = 1;
    for(int i = count + 1; i< len_str; i++){
        name_file = realloc(name_file, sizeof(char) *  (i - count));
        name_file[i - count - 1] = str[i];
...
void parsingNum(char* str, int len_str, char* name_file, char num[3]){
    for(int i = 0; i < 3; i++){
        num[i] = str[i];
    }
    int count = 3;
...
mode_t maskCalcNum(char num[3]){
    return (mode_t) strtol(num, NULL, 8) & 0777;
...
int main(){
    ...
    if (mode_num == 3){
        ...
        parsingPrint(str, len_str, name_file);
        FileMod(name_file);
    }
    else{
        char num[3];
        parsingNum(str, len_str, name_file, num);
        FileMod(name_file);
        printMode(maskCalcNum(num));
    }
